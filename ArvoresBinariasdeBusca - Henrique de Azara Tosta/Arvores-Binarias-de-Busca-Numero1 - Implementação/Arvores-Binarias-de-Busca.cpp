...
void ImprimirNosNaoFolha(TipoApontador *p){
	if(*p != NULL){
		ImprimirNosNaoFolha(&(*p)->Esq);
		if((*p)->Esq != NULL || (*p)->Dir != NULL){
			printf("%d\n", (*p)->Registro.Chave);
			ImprimirNosNaoFolha(&(*p)->Dir);
		}
	}
}

void ImprimirTodosNosFolha(TipoApontador *p){
	if (*p != NULL){
		ImprimirTodosNosFolha(&(*p)->Esq);
		if((*p)->Esq == NULL && (*p)->Dir == NULL){
			printf("%d\n", (*p)->Registro.Chave);
		}
		ImprimirTodosNosFolha(&(*p)->Dir);
	}
}

void VerificarBinaria(TipoApontador *p){
	if(*p != NULL){
		VerificarBinaria(&(*p)->Esq);
		if(((*p)->Esq == NULL && (*p)->Dir == NULL) || ((*p)->Esq != NULL && (*p)->Dir != NULL)){
			VerificarBinaria(&(*p)->Dir);
		}else{
